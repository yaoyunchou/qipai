"""在 Supabase 上执行 sql/supabase/01-init-schema.sql。用法: python -m scripts.apply_schema"""
import sys
...
SQL_FILE = ROOT.parent.parent / "sql" / "supabase" / "01-init-schema.sql"
...
def _resolve_database_url() -> str:
    ...
    if not settings.database_url:
        print("错误：请在 backend/.env 中配置 DATABASE_URL（参考 .env.example）", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    ...
    print("下一步：python -m scripts.init_admin  创建 admin 账号")
