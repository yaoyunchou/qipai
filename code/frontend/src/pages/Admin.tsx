...
export default function AdminPage() {
  ...
  const configTab = (
    <Card title="基准价、计时计费与权限">
      {cfg && (
        <Form form={form} layout="vertical" onFinish={saveConfig} initialValues={cfg} disabled={!isAdmin}>
          <Form.Item name="enable_timing" label="启用计时自动计费" valuePropName="checked">
            <Switch />
          </Form.Item>
          <Form.Item
            noStyle
            shouldUpdate={(prev, cur) => prev.enable_timing !== cur.enable_timing}
          >
            {({ getFieldValue }) =>
              getFieldValue("enable_timing") ? (
                <>
                  <Form.Item
                    name="base_price"
                    label="单价（元/计费单位）"
                  ...
                  <Form.Item name="billing_unit_minutes" label="计费单位（分钟）">
                    <InputNumber min={1} max={1440} style={{ width: "100%" }} placeholder="60=每小时" />
                  </Form.Item>
                  <Form.Item name="min_billing_units" label="最低计费单位数">
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            render: (_, row) => (
              <Space size={0}>
                <Button type="link" size="small" onClick={() => openEditTable(row)}>
                  编辑
                </Button>
                <Button type="link" danger size="small" onClick={() => deleteTable(row)}>
                  删除
                </Button>
              </Space>
            ),
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            render: (_, row) => (
              <Space size={0}>
                <Button type="link" size="small" onClick={() => resetUserPassword(row)}>
                  重置密码
                </Button>
                <Button
                  type="link"
